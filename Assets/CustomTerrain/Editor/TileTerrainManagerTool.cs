...
{
    public class TileTerrainManagerTool : TerrainPaintTool<TileTerrainManagerTool>
    {
        private class Styles
        {
            public GUIContent fillHeightmapUsingNeighbors = EditorGUIUtility.TrTextContent("使用接邻地型填充高度图", "通过接邻地型的数据对高度图进行交叉混合");
            public GUIContent fillAddressMode = EditorGUIUtility.TrTextContent("地址填充模式", "邻接地型的地址采样模式类型");
            public GUIContent terrainToolPropertyChange = EditorGUIUtility.TrTextContent("地型工具的属性修改");
        }
        ...
        Terrain CreateNeighbor(Terrain parent, Vector3 position)
        {
            ...
            var assetsToSave = new Object[1 + terrainData.alphamapTextureCount];
            ...
            TerrainExpand exp = terrainGO.AddComponent<TerrainExpand>();

            Undo.RegisterCreatedObjectUndo(terrainGO, "Add New Terrain Tile");
